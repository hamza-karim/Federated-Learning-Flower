...
1. Build the docker image:
   ```bash
   docker build --platform linux/arm64 -t hamzakarim07/flwr_server_hfl:latest -f FL_server/docker/Dockerfile .
   ```
2. Push the docker image to docker hub:
   ```bash
   docker push hamzakarim07/flwr_server_hfl:latest
   ```
3. Pull the Docker image for the server on the Jetson Nano device from [Docker Hub](https://hub.docker.com/repositories/hamzakarim07):
   ```bash
   sudo docker pull hamzakarim07/flwr_server_hfl:latest
4. Run following command to run docker container:
   ```bash
   sudo docker run -d --name flwr-server_hfl --runtime=nvidia --gpus all -e NVIDIA_VISIBLE_DEVICES=all -p 8080:8080 hamzakarim07/flwr_server_hfl:latest
...
3. Pull the Docker image for the client on the Jetson Nano device from [Docker Hub](https://hub.docker.com/repositories/hamzakarim07):
   ```bash
   sudo docker pull hamzakarim07/flwr_client_hfl:latest
4. Run following command to run docker container for client 1, just change the name of container for client 2:
   ```bash
   sudo docker run -d --name flwr-client1 --runtime=nvidia --gpus all -e NVIDIA_VISIBLE_DEVICES=all hamzakarim07/flwr_client_hfl:latest
5. Go inside the docker container:
   ```bash
   sudo docker exec -it <container_id_or_name> bash
   cd src
6. Run python command to execute the python script for client 1 and 2 **--ip is server ip**:
   ```bash
   python3 client_LSTM_2.py --ip=<Server_IP> --folder=Client_1_LSTM --id=1
   python3 client_LSTM_2.py --ip=<Server_IP> --folder=Client_22_LSTM --id=2
   python3 client_LSTM_2.py --ip=<Server_IP> --folder=Client_2_LSTM --id=3

7. After using copy.sh, use below command to copy images from host to local machine
   ```bash
...
5. Adding SSH-Key in edge devices:
   ```bash
   ssh-keygen -t rsa -b 4096 -C "hamza_karim_edge_key"
   ssh-copy-id c2sragx04@10.226.47.85
...
4. SSH to use portainer:
   ```bash
   ssh -L 9443:localhost:9443 c2srnano07@10.226.47.97
